...
exports.getRestaurants = function(next) {
    var restaurants = require('./generated-restaurants.json');
    next(null, restaurants);

};

exports.getRestaurantDetails = function(restId, next) {
  var restaurantDetails = require('./restaurant-menu.json');
    var restaurantDetail = restaurantDetails.filter(function(rest) {
      return rest.res_id==restId;
    });
    next(null, restaurantDetail);
};
...
exports.placeOrder = function(order_id, card, next) {
  ...
  Order.findOne({_id: order_id}, function(err, order) {
    ...
    var result = Math.floor(Math.random() * 1000) + 1000;
    if (!err) {
        console.log(result);
        order.refnum = result;
